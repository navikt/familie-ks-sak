...
fun <T : EøsSkjema<T>> T.utenInnholdTom(tom: YearMonth?) =
    this.kopier(fom = this.tom?.plusMonths(1), tom = tom).utenInnhold()
...
fun <T : EøsSkjema<T>> T.erLikBortsettFraTom(skjema: T): Boolean =
    this.kopier(tom = skjema.tom) == skjema

fun <T : EøsSkjema<T>> T.erLikBortsettFraBarn(skjema: T): Boolean =
...
fun <T : EøsSkjema<T>> T.medFjernetBarn(skjema: T): T =
