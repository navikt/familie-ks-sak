...
internal fun Iterable<EndretUtbetalingAndel>.tilBarnasSkalIkkeUtbetalesTidslinjer(barnaAktør: Set<Aktør>): Map<Aktør, Tidslinje<Boolean>> =
    this
        .filter { it.årsak in listOf(Årsak.ETTERBETALING_3MND, Årsak.ALLEREDE_UTBETALT) && it.prosent == BigDecimal.ZERO }
        .flatMap { andel ->
            andel.aktører
                .filter { it in barnaAktør }
                .map { it to andel }
