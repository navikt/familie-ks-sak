...
For å bygge tjenesten lokalt så kreves det at man har satt visse miljø
```
./gradlew clean build 
```
...
* `DevLauncherPostgresPreprod`. Kjører mot intergrasjoner og pdl i preprod(ikke q1, men syntetisk). Har støtte for å
  ...
  CLIENT_SECRET må settes til familie-ks-sak sin azure client id og secret for å få tilgang til pdl og integrasjoner.
...
fra [Vault](https://vault.adeo.no/ui/vault/secrets/kv%2Fpreprod%2Ffss/show/familie-ks-sak/default).
...
Du kan bruke Postman til å kalle APIene i ks-sak. Det krever at du har satt opp [autentisering](#autentisering) riktig,
og har et token som gjør at du kaller som ks-sak-frontend.

Den nødvendige informasjonen for å få frontend-token'et får du ved å kalle:

`kubectl -n teamfamilie get secret azuread-familie-ks-sak-frontend-lokal -o json | jq '.data | map_values(@base64d)'`.
...
* Body: `x-www-form-urlencoded` med følgende key-values
    ...
    * `scope`: `api://dev-gcp.teamfamilie.familie-ks-sak-lokal/.default
...
pm.test("Lagre token globalt", function () {
    ...
    pm.globals.set("azure-familie-ks-sak", jsonData.access_token);
});
```

som vil plukke ut token'et og lagre det i en global variabel, her `azure-familie-ks-sak`

Når du lager kall mot APIet, så kan du sette følgende i header'en for å sende med token'et:

* `Authorization`: `Bearer {{azure-familie-ks-sak}}`
...
Det kan være praktisk å teste ut api'et i et verktøy som Postman. Da må vi late som vi er familie-ks-sak-frontend.

Oppsettet består av tre deler:

* Conf'e ks-sak riktig
* Få tak i et gyldig token
* Sende en request med et token'et

### Conf'e ks-sak riktig

Vi trenger å sette tre miljøvariable i familie-ks-sak:

* `BA_SAK_CLIENT_ID`: Id'en til familie-ks-sak
* `CLIENT_SECRET`: Hemmeligheten til familie-ks-sak
...
I postman lagrer du følgende request, som gir deg et token som familie-ks-sak-frontend for å kalle familie-ks-sak:
...
* `grant_type`: `client_credentials`
* `client_id`: <`AZURE_APP_CLIENT_ID` for familie-ks-sak-frontend>
* `client_secret`: <`AZURE_APP_CLIENT_SECRET` for familie-ks-sak-frontend>
* `scope`: `api://dev-fss.teamfamilie.familie-ks-sak-lokal/.default`
...
Lagre gjerne request'en med et hyggelig navn, f.eks 'Token familie-ks-sak-frontend -> familie-ks-sak'

### Sende en request med et token'et

1. `Headers`: Her MÅ du ha `Authorization=Bearer {{azure-familie-ks-sak}}`, altså referanse til token'et som ble lagret
   i scriptet over
2. `Verb`: F.eks `GET`
...
* 'Token familie-ks-sak-frontend -> familie-ks-sak', for å få token
